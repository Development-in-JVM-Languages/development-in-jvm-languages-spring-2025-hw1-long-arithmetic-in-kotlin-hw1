...
class BigInt(private val value: String) : Comparable<BigInt> {
    ...
    operator fun minus(other: Byte): BigInt {
        ...
        // If the other number is zero, return this number
        if (otherBigInt.sign() == 0) {
            return this
        }

        // Negate the other number and add it to this number
        return this + (-otherBigInt)
    }
    ...
    private fun divideDigits(a: List<Int>, b: List<Int>): Pair<List<Int>, List<Int>> {
        // Handle cases where the divisor is larger than the dividend
        if (compareDigits(a, b) < 0) {
            return Pair(listOf(0), a)
        }
    ...
    operator fun rem(other: BigInt): BigInt {
        ...
        // Compute the remainder using the divideDigits function
        val (_, remainderDigits) = divideDigits(digits, other.digits)

        // Create the result BigInt with the sign of the dividend
    ...
    fun pow(exp: Int): BigInt {
        return  pow(BigInt(exp.toString()))
    }
    ...
    override fun toString(): String {
        return when (sign) {
            -1 -> "-${digits.joinToString("")}" // Negative number
            0 -> "0" // Zero
            else -> digits.joinToString("") // Positive number
        }
    }
    ...
    operator fun compareTo(other: Int): Int {
        return compareTo(BigInt(other.toString()))
    }
    ...
    override fun hashCode(): Int {
        return digits.hashCode() + sign.hashCode()
    }
